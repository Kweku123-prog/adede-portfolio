...
export default function GadedeSegbefiaPortfolio() {
  ...
  return (
    <div className="min-h-screen text-gray-200 bg-[#0a0a0f] font-sans antialiased">
      {/* NAV */}
      <header className="fixed w-full z-40 top-3 left-0 px-4 sm:px-6">
        <nav className="backdrop-blur-md bg-black/40 border border-white/10 rounded-2xl shadow-lg mx-auto max-w-6xl p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center justify-between gap-3">
            <button onClick={() => scrollTo("home")} aria-label="Go home" className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-rose-400 shadow shrink-0">
                <img src="/assets/ITC-90.jpg" className="w-full h-full object-cover" />
              </div>
              <div className="text-xs sm:text-sm">
                <div className="font-semibold text-white leading-tight">Gadede Aku Segbefia</div>
                <div className="text-[11px] sm:text-xs text-gray-300">Actor · Voice Over</div>
              </div>
            </button>
            <a href="mailto:gadedesegbefia1@gmail.com" className="md:hidden px-3 py-2 rounded-lg bg-rose-600 text-white text-xs font-medium hover:bg-rose-700">Book</a>
          </div>
          ...
          <div className="md:hidden flex items-center gap-2 overflow-x-auto pb-1">
            {nav.map((n) => (
              <button
                key={n.id}
                onClick={() => scrollTo(n.id)}
                className={`shrink-0 px-3 py-2 rounded-full text-xs font-medium transition-all ${active === n.id ? "text-rose-300 bg-white/10" : "text-gray-300"}`}
              >
                {n.label}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <a href="mailto:gadedesegbefia1@gmail.com" className="px-4 py-2 rounded-lg bg-rose-600 text-white text-sm font-medium hover:bg-rose-700">Book / Contact</a>
          </div>
        </nav>
      </header>

      <main className="pt-36 sm:pt-32 md:pt-28">

        {/* HOME */}
    <section
  id="home"
  ref={(el) => sectionRef(el, "home")}
  className="relative w-full px-4 sm:px-6 py-20 sm:py-28 min-h-screen flex items-center bg-black"
>
  {/* Background Video */}
  <div className="absolute inset-0 overflow-hidden">
    <iframe
      src="https://player.vimeo.com/video/1143827126?background=1&autoplay=1&loop=1&muted=0&autopause=0"
      frameBorder="0"
      allow="autoplay; fullscreen; picture-in-picture"
      className="absolute inset-0 w-full h-full scale-[1.6] sm:scale-[1.3] md:scale-100 origin-center"
    ></iframe>
  ...
  {/* Foreground content */}
  <div className="relative grid grid-cols-1 md:grid-cols-2 gap-10 items-center z-10 text-center md:text-left">
    <motion.div
    ...
    >
      ...
      <h1 className="mt-4 text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight text-white">
        ...
        <span className="block text-xl sm:text-2xl md:text-3xl text-gray-200 mt-2">
          Stage & Screen Performer
        </span>
      </h1>
      <p className="mt-4 sm:mt-6 text-base sm:text-lg text-gray-300 italic">
        "Doing the Lord's work through arts."
      </p>
      <p className="mt-4 text-sm sm:text-base text-gray-300 max-w-xl md:mx-0 mx-auto">
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:justify-start justify-center">
        <button
          ...
          className="px-5 py-3 rounded-lg bg-rose-600 text-white shadow hover:bg-rose-700 w-full sm:w-auto"
        >
          About Gadede
        </button>
        <button
          onClick={() => scrollTo("media")}
          className="px-5 py-3 rounded-lg border border-rose-400 text-gray-200 hover:bg-white/10 w-full sm:w-auto"
        >
          View Media
        </button>
      </div>
...
<p className="text-rose-400 font-semibold text-lg sm:text-xl mt-6">— Gadede Aku Segbefia</p>
        ...
        <section id="about" className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="mt-4 text-sm sm:text-base text-gray-300">Gadede Aku Segbefia is a Ghanaian actor and voice over artiste with over 10 years of professional experience. She is a freelance performer and a guest actor with the Ghana Drama Company (Abibigroma), resident at the National Theatre of Ghana.
...
Today, Gadede’s work spans stage, screen, voice performance, and commercial storytelling. Her artistry is shaped by truth, freedom, and a deep belief in the power of narrative to heal, inspire, and transform.
            ...
            <p className="mt-4 text-sm sm:text-base text-gray-300">Her performances balance emotional depth, faith-driven passion, and artistic discipline.</p>
          </div>
        </section>
...
<section id="experience" className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
...
<ul className="mt-4 space-y-3 text-sm sm:text-base text-gray-300">
...
<section id="experience" className="max-w-6xl mx-auto px-6 py-24">
        ...
        <section id="media" ref={(el) => sectionRef(el, "media")} className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
          ...
          <div className="mt-8 sm:mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {["ITC-70.jpg", "ITC-10.jpg", "ITC-115.jpg", "ITC-5.jpg", "ITC-151.jpg", "ITC-122.jpg", "PRM_3764.jpg", "PRM_3954-Edit.jpg","PRM_3962-Edit.jpg"].map((img) => (
              <div key={img} className="rounded-2xl overflow-hidden shadow-lg bg-white/5 border border-white/10">
                <img src={`/assets/${img}`} className="w-full h-56 sm:h-64 object-cover" />
              </div>
            ))}
          </div>
        </section>
        ...
        {/* VIDEOS */}
        <section id="videos" ref={(el) => sectionRef(el, "videos")} className="max-w-6xl mx-auto px-4 sm:px-6 py-20">
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-6">
            <div className="rounded-2xl overflow-hidden shadow-lg">
              <iframe className="w-full h-48 sm:h-64" src="https://www.youtube.com/embed/tgbNymZ7vqY" allowFullScreen></iframe>
            </div>
            <div className="rounded-2xl overflow-hidden shadow-lg">
              <iframe className="w-full h-48 sm:h-64" src="https://www.youtube.com/embed/dQw4w9WgXcQ" allowFullScreen></iframe>
            </div>
          </div>
        </section>
        ...
        {/* VOICE */}
   {/* VOICE */}
<section id="voice" ref={(el) => sectionRef(el, "voice")} className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
...
<div className="mt-10 grid md:grid-cols-2 gap-8">
  {[
    ...
    <div key={v} className="rounded-2xl bg-white/5 border border-white/10 p-6">
      <audio controls autoPlay preload="auto" className="w-full">
        ...
        {/* CONTACT */}
        <section id="contact" ref={(el) => sectionRef(el, "contact")} className="max-w-6xl mx-auto px-4 sm:px-6 py-20">
          <div className="bg-black/40 border border-white/10 rounded-2xl p-6 sm:p-10 shadow-lg">
